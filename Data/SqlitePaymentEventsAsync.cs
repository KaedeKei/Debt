...
namespace Shedule.Data
{
	public class SqlitePaymentEventsAsync : IPaymentEvent
    {
		...
		public async Task<PaymentEvent> GetSame(PaymentEvent checkEvent)
		{
			var thisEvent = await _dbContext.PaymentEvents.FirstAsync(it => it.CustomerId == checkEvent.CustomerId && it.PaymentDate == checkEvent.PaymentDate && it.PaymentSumm == checkEvent.PaymentSumm && it.PaymentText == checkEvent.PaymentText);
			return thisEvent;			
		}
